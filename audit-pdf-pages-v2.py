...
pages = {
    ...
    "Page 5 — Camera Intelligence Opener (1910s Box Camera)": "/Users/claudiabot/.openclaw/media/pdf-page-5.png",
    "Page 17 — Techniques Opener (1960s Darkroom)": "/Users/claudiabot/.openclaw/media/pdf-page-17.png",
    "Page 22 — CTA Bookend (1920s Factory reuse)": "/Users/claudiabot/.openclaw/media/pdf-page-22.png",
}

AUDIT_PROMPT = """You are a senior art director at a top design agency, evaluating PDF pages for a B2B lead magnet against strict design standards.

{design_truth}

ANALYZE THIS PAGE: "{page_name}"

This page uses a full-bleed vintage photograph as background with a dark overlay, and white/light text on top. It's meant to feel like an editorial magazine section opener (think Esquire, GQ, National Geographic).

Score each criterion 1-10 and provide SPECIFIC, ACTIONABLE feedback. Be brutally honest — point out every flaw:

1. **Typography & Hierarchy** (1-10): Is the type hierarchy clear? Are fonts readable over the background? Size ratios appropriate? Any text that's hard to read?
2. **Color & Contrast** (1-10): Is the overlay opacity right — can you read ALL text easily? Does the image show through enough to be impactful, or is it too dark/too light? Any hotspots where text gets lost?
3. **Composition & Layout** (1-10): Is the text placement intentional? Does it follow rule of thirds? Is there sufficient negative space? Does the eye flow naturally?
4. **Image-Text Integration** (1-10): Does the background image enhance the content or fight it? Does it feel like an editorial magazine spread? Or does the text just sit awkwardly on top of the photo?
5. **Brand Consistency** (1-10): Does it feel like the same brand system? ICG red accent used appropriately?
6. **The Gut Test** (1-10): In 2-3 seconds, does this page communicate professional, premium quality? Or does it look like a template?
...
OVERALL SCORE: X/10
TOP 3 ISSUES (most critical first, with specific fix recommendations):
...
report = "# PDF Photo Pages — Design Audit (Gemini Vision vs DESIGN_TRUTH)\n\n" + "\n\n---\n\n".join(results)
